...
metadata:
  hermes:
    ...
    triggers: [twitter, x-api, social-media, tweet, dm, thread-reader, url-extraction]
...
```bash
xurl --app APP_NAME '/2/tweets/<discord-channel-id>?expansions=author_id,attachments.media_keys,referenced_tweets.id&tweet.fields=created_at,lang,public_metrics,context_annotations,entities,possibly_sensitive,conversation_id,in_reply_to_user_id,referenced_tweets,article'
```
