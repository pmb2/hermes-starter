...
metadata:
  hermes:
    tags:
      ...
      - generative-ai
      - video-generation
    triggers: [comfyui, image-generation, stable-diffusion, flux, video-generation, ai-art, workflow-automation, creative-ai]
    related_skills: [pixel-art, architecture-diagram]
    category: creative
---
...
11. **Model downloads inside Docker + Windows** — `comfy model download` and `hf download` use the `hf` CLI internally, which can hit MSYS path translation issues on Windows Docker Desktop. Files end up in `C:/Program Files/Git/workspace/...` (nonexistent paths inside the container). **Fix:** download models directly inside the container via `docker exec yt-anim-comfyui huggingface-cli download ...` with `HF_TOKEN` set. Docker cp for files >4 GB across the MSYS boundary silently corrupts them — use the temp-container volume-mount technique from pitfall #15 instead. See `references/flux-schnell.md` in this skill for model size reference.
...
13. FLUX.1-schnell CFG MUST be 1.0 - Using ComfyUI default CFG (~7.0 for SD1.5/SDXL) with FLUX.1-schnell produces solid-color garbage images. FLUX uses guidance-free architecture. Always set CFG to 1.0 in workflow templates. This is the single most common cause of bad FLUX images.

14. FLUX model name file vs directory collision - flux1-schnell-fp8 exists as BOTH a file (flux1-schnell-fp8.safetensors, ~11GB) AND a directory (flux1-schnell-fp8/) in checkpoints/. When workflow specifies ckpt_name with .safetensors extension, ComfyUI loads the file. Without extension, it may pick the directory and fail silently. Fix: Always use full filename with .safetensors extension.

15. VRAM saturation on Windows WDDM - On Windows with Docker Desktop + NVIDIA, system VRAM (24GB RTX 3090) can show 23.9GB in use from desktop apps: Chrome, Firefox, Discord, ChatGPT, Codex, Docker WSL2, Edge WebView2. Use --lowvram when VRAM >90% occupied. Kill Docker Desktop process to flush GPU state after hangs.
    - **Diagnose:** Run `nvidia-smi --query-gpu=memory.used --format=csv,noheader` before attempting any run.
    - **Mitigation:** Use ComfyUI's `--lowvram` flag instead of `--normalvram` when VRAM is >90% occupied. With `--lowvram`, images generate in 2–5 min per frame on an RTX 3090 (model swaps between GPU and system RAM). With `--normalvram` under VRAM pressure, the CUDA driver can hang the entire Docker container.
    - **Restoration:** If Docker containers freeze from GPU hangs, kill Docker Desktop via Windows (`Stop-Process -Name 'Docker Desktop' -Force`) and restart — this flushes the GPU driver state and frees all VRAM.
    - **Best fix:** Close browser tabs and GPU-accelerated apps before running FLUX inference. Freeing 4–6 GB VRAM makes `--normalvram` viable and drops per-frame time to 10–15 seconds.

15. **Docker GPU passthrough on Windows is fragile** — `docker cp` for files >4 GB across the MSYS/bash boundary corrupts silently (archive/tar write-too-long errors masked by exit code 0). Use `docker run` with a temporary container that mounts both the Docker volume (`-v source:/source:ro`) and a host bind mount (`-v /host/path:/dest`) to copy large model files. Even this times out for very large models (6–16 GB) on Docker Desktop Windows due to slow filesystem bridging. The most reliable approach is downloading models directly inside the container via `huggingface-cli download` with a mounted `HF_TOKEN`, or using HuggingFace's Inference API serverless endpoint instead of local GPU.

16. **Corrupted FLUX model — Safetensor Deserialization Error**
    - **Symptom**: ComfyUI log shows `safetensors_rust.SafetensorError: Error while deserializing header: incomplete metadata, file not fully covered`. Model file exists at expected path but loading fails.
    - **Root cause**: Incomplete/corrupted download. Docker `curl -L` with `--max-time` set too short truncates the file. Docker cp >4GB also silently corrupts.
    - **Diagnose**: Compare file size against expected. `flux1-schnell-fp8.safetensors` = ~11GB (Comfy-Org variant) or ~6.2GB (original HF). A significantly smaller file is truncated.
    - **Fix**: Re-download with `-C -` (resume) and `--max-time 7200`. Or use `huggingface_hub` Python library inside the container for reliable downloads (proper HTTP range requests, retry, resume built in):
      ```bash
      docker exec yt-anim-comfyui sh -c "pip install huggingface_hub && python -c '
      from huggingface_hub import hf_hub_download
      hf_hub_download(repo_id=\"Comfy-Org/flux1-schnell\", filename=\"flux1-schnell-fp8.safetensors\",
                      local_dir=\"/workspace/ComfyUI/models/checkpoints/\",
                      local_dir_use_symlinks=False, resume=True)
      '"
      ```
    - **Alternative**: Drop-in replacement: use `flux1-dev-fp8.safetensors` (from Kijai/flux-fp8, 12GB) with the same workflow — just change `ckpt_name` and increase steps to 20+.
