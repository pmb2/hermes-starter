...
metadata:
  hermes:
    ...
    triggers: [humanize, text, writing, editing, anti-ai, prose, voice, tone, ai-detection]
    related_skills: [business-voice-outreach, songwriting-and-ai-music]
    category: creative
...
- **Pre-built profiles:** For recurring users, voice calibration profiles live under this skill's `references/` directory (e.g. an `outreach-voice-profile.md` you author) — loaded automatically when `humanizer` is loaded for that user.

### Proactive Voice Ingestion (Gmail/IMAP)

When a user wants you to learn their voice at scale but has no prepared samples:

1. **Set up IMAP access** to their Gmail account using an App Password:
   - User enables 2FA → generates App Password in Google Account → Security
   - Install himalaya CLI or use Python's `imaplib` for direct access
   - The App Password contains spaces — strip them: `password.replace(" ", "")`
   - Gmail Sent folder name: `'"[Gmail]/Sent Mail"'` (must be double-quoted due to brackets)

2. **Pull sent emails** — fetch the most recent 100-200 from `[Gmail]/Sent Mail`.

3. **Filter out auto-generated/system emails** — skip cron reports, Job Agent summaries, heartbeats, digests, or anything with templated subjects.

4. **Analyze the remaining human-written emails**:
   - Extract plain text body (strip HTML, quoted reply blocks, forwarded message blocks)
   - Count sentence/paragraph length distribution
   - Identify common openers/closers, signature blocks, contraction frequency
   - Build a voice profile JSON with: do-use patterns, avoid patterns, signature phrases, tone markers

5. **Calibrate the humanizer** — every subsequent rewrite references the profile.

6. **Test & iterate** — write a sample in the user's voice, show them, adjust based on feedback.

**Pitfalls:**
- On MSYS/Windows, `~` expands to the Hermes profile's home dir, not `C:\Users\<user>`. Use absolute paths in scripts.
- `imaplib` on Windows requires the Gmail folder name to be double-quoted: `mail.select('"[Gmail]/Sent Mail"')`
- App Passwords with spaces must be set as quoted env vars or stripped in code.
- The `imap_tools` library is cleaner than raw `imaplib` but may not be installed.
