...
metadata:
  hermes:
    ...
    triggers: [build-page, stripe-style, linear-style, vercel-style, landing-page, dashboard-design, web-design-ui]
    related_skills: [claude-design, sketch, architecture-diagram, excalidraw]
---
...
- **`ui-ux-pro-max`** — use when the deliverable is a full-application redesign
  in a framework (Next.js/shadcn). See Mode B above. This skill supplies the
  visual vocabulary; `ui-ux-pro-max` covers the multi-page orchestration pattern
  and design token definition workflow.
...
### Mode A: Generate a standalone HTML page
...
### Mode B: Design inspiration for an existing framework app (Next.js/shadcn)

When redesigning the UI of an existing Next.js + Tailwind + shadcn application, use templates as visual direction, not as copy-paste HTML source:

1. Pick 1-2 templates matching the app's domain (dashboards → Linear/Sentry/Vercel, landing pages → Stripe/Apple/Airbnb)
2. Load the templates and extract concrete values: hex colors, border radiuses, shadow technique, font weights, component patterns
3. Define the design system natively in the framework: update `tailwind.config.ts` CSS variables, `globals.css` HSL tokens, and `layout.tsx` with next/font
4. Rewrite pages using the designed tokens (see `ui-ux-pro-max` → Step 2c for the parallel subagent pattern)
5. Verify with `next build`, not `browser_vision` — the design lives in the codebase now, not a standalone artifact

**When to use Mode B:** The user has an existing project in a framework and asks to "redesign the entire UI", "make it look modern", or "apply a consistent design system across all pages". The templates supply the visual vocabulary; your shading/tailwind config supplies the implementation.

**When to use Mode A:** The user wants a standalone landing page, HTML mockup, or one-off artifact without a framework.
...
| `qa-lead.md` | Sentry | Dark dashboard, data-dense, pink-purple accent |
...
| Template | Site | Style |
|---|---|---|
| `airtable-design.md` | Airtable | Colorful, friendly, structured data aesthetic |
...
| `spotify-design.md` | Spotify | Vibrant green on dark, bold type, album-art-driven |
