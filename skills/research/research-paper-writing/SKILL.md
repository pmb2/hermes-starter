...
metadata:
  hermes:
    ...
    triggers: [paper-writing, research-paper, ml-paper, neurips, icml, iclr, latex, publication, submission, academic-writing, scientific-writing, experiment-design, literature-review]
    category: research
    related_skills: [gpt-researcher, open-coscientist, arxiv, ai-scientist, subagent-driven-development, plan]
...
- Colorblind-safe palettes (Okabe-Ito or the operator Tol)
...
> Full details in [references/phase5-paper-drafting.md](references/phase5-paper-drafting.md) — this section provides a condensed overview.

Covers: Context management for large projects, the narrative principle, sources, time allocation, writing workflow (12-step checklist), two-pass refinement pattern, LaTeX error checklist, title & abstract writing (5-sentence formula), Figure 1 strategy, introduction structure, methods documentation, experiments & results reporting, related work organization, limitations statement, conclusion & discussion, appendix strategy, ethics & broader impact statement, datasheets & model cards, page budget management, writing style (Gopen & Swan), LaTeX templates (all venues), professional preamble packages (microtype, siunitx, cleveref, tikz, algorithm2e), tables & figures (booktabs, subfigures, Okabe-Ito palette), TikZ diagram patterns (pipeline, matrix, iterative loop), latexdiff revision tracking, SciencePlots matplotlib styling, and conference resubmission guidance.

Key principles:
- **Context management**: Load only what each drafting task needs; use experiment_log.md as the primary context bridge
- **Pass 1 refinement**: Write + immediate refine per section to catch local issues
- **Pass 2 refinement**: Full-paper-aware review to fix cross-section inconsistencies
- **Figures first**: Draft Figure 1 before the introduction — it forces clarity
- **LaTeX hygiene**: Always compile after each section; never modify .sty files
- **Appendix-first**: Put proofs, extra experiments, and implementation details in the unlimited appendix
...
**Goal**: Final checks, formatting, and submission preparation for conference venues.

> Full details in [references/submission-preparation.md](references/submission-preparation.md) — this section provides a condensed overview.

Covers: conference checklists, anonymization, formatting verification, LaTeX pre-compilation validation, final compilation, conference-specific requirements, resubmission/format conversion, camera-ready preparation, arXiv preprint strategy, and research code packaging.

Key checkpoints:
- **Pre-submission checklist**: Complete venue-specific mandatory checklists (see [references/checklists.md](references/checklists.md))
- **Anonymization**: Verify all 10 points in the anonymization checklist
- **Formatting**: Check page limits, vector figures, grayscale readability, references
- **Pre-compilation validation**: Run `chktex` + citation/figure/label checks before `pdflatex`
- **Conference-specific**: Each venue (NeurIPS, ICML, ICLR, ACL, AAAI, COLM) has unique requirements
- **Camera-ready**: De-anonymize, add acknowledgments, switch to camera-ready template
- **arXiv strategy**: Post after submission for double-blind venues, choose correct categories
- **Code packaging**: Include README, LICENSE, reproduction scripts, environment config
...
Workshop and short papers follow a compressed version of the main pipeline. See [references/workshop-papers.md](references/workshop-papers.md) for venue-specific strategies, page limits, and submission guidance.

---

## Paper Types Beyond Empirical ML

The main pipeline above targets empirical ML papers. Other paper types require different structures and evidence standards. See [references/paper-types.md](references/paper-types.md) for detailed guidance on each type, including theory papers, survey/tutorial papers, benchmark/dataset papers, and position papers.
...
| Paper rejected, need to resubmit | See [references/submission-preparation.md](references/submission-preparation.md) (Conference Resubmission). Address reviewer concerns without referencing reviews. |
| Missing broader impact statement | See [references/phase5-paper-drafting.md#step-510-ethics--broader-impact-statement](references/phase5-paper-drafting.md) (Phase 5). Most venues require it. "No negative impacts" is almost never credible. |
...
| Reviewers question reproducibility | Release code (see [references/submission-preparation.md](references/submission-preparation.md)), document all hyperparameters, include seeds and compute details. |
